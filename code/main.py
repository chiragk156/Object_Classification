...
if __name__=="__main__":
    ...
    print('VGG data read done')
    # Train SVM
    # svm_classifier = trainSVM(X,y,0.2)
    file = open('svm','rb')
    svm_classifier = pickle.load(file)
    file.close()
    print('SvM loading done')
    img = cv2.imread('../VOC2012test/VOCdevkit/VOC2012/JPEGImages/2008_000653.jpg')
    rects_list = select_regions(img)
    print('recatangles selection done')
    out = maximum_region_selector(svm_classifier,img,rects_list,classes)
