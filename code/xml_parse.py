...
def xml_parse_testing(classes, main_dir_path, xml_dir_path, images_dir_path, test_cropped_img_dir_path):
    #making cropped images directory if not exists
    directory = os.path.dirname(test_cropped_img_dir_path)
    if not os.path.exists(directory):
        os.mkdir(directory)

    for obj in classes:
        #making cropped images object directory if not exists for saving cropped images
        directory = os.path.dirname(test_cropped_img_dir_path + obj + '/')
        if not os.path.exists(directory):
            os.mkdir(directory)
           
        test_file = main_dir_path + obj + '_test.txt'             #train images text file name 
        file = open(test_file)                                     #opening training text file
        for line in file:                                           #reading text file and finding images with required object in them
            line = line.strip('\n')
            tokens = line.split(" ")
            if (len(tokens)==3 and tokens[2]=='1'):                 #if object image name is found
